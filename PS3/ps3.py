...
class Position(object):
    ...
    def get_floored_position(self):
        return Position(math.floor(self.x), math.floor(self.y))
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        #dictionary with tiles => dirt_amount
        self.tiles = {}
        #Create every location
        for w in range(width):
            for h in range(height):
                self.tiles[Position(w,h)] = dirt_amount
    ...
    def is_tile_cleaned(self, w, h):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.tiles[Position(w,h)] == 0
    ...
    def get_dirt_amount(self, w, h):
        ...
        """
        for tile in self.tiles:
            if (Position(w,h) == tile):
                return self.tiles[tile]

        raise ValueError("Given position is not in the room")
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.location = room.get_random_position()
        self.room = room
        self.speed = speed
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        if (pos.x < self.w and pos.y < self.h):
    ...
    def is_position_valid(self, pos):
        ...
        """
        if (pos.x < self.w and pos.y < self.h):
            return not is_position_furnished(pos)
    ...
    def get_random_position(self):
        ...
        """
        while 1:
            new_position = random.choice(self.tiles.keys())
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_position = self.location.get_new_position(self.direction,self.speed)
        new_position_floored = new_position.get_floored_position()
        print(new_position)
        #check if it's unfurnished
        if (self.room.is_position_valid(new_position_floored)):
            self.room.clean_tile_at_position(new_position_floored,self.capacity)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        print(new_position)
        #check if it's unfurnished
        if (self.room.is_position_valid(new_position_floored) and not self.gets_faulty()):
            self.room.clean_tile_at_position(new_position_floored,self.capacity)
            self.set_robot_position(new_position)
        else:
            self.set_robot_direction(random.randint(0,359))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials, robot_type):
    ...
    """
        ...
        for trial in range(1):
            ...
            cleaningRobot = StandardRobot(room,5,10)

            while room.get_num_cleaned_tiles() != room.get_num_tiles():
                cleaningRobot.update_position_and_clean()
                print(str(cleaningRobot.location.x) +", "+ str(cleaningRobot.location.y))
                seconds +=1
